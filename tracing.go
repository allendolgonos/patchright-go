...
func (t *tracingImpl) Start(options ...TracingStartOptions) error {
	// Ensure injection route is installed before tracing to allow script injection
	if parent := t.parent; parent != nil && parent.channel != nil {
		if ctx, ok := parent.channel.object.(*browserContextImpl); ok {
			_ = ctx.installInjectRoute()
		}
	}
