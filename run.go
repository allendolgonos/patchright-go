...
// resolvePatchrightVersion determines which driver version to use.
// Precedence:
// 1) PATCHRIGHT_DRIVER_VERSION env var
// 2) latest release tag from Patchright GitHub releases
// 3) fallback to upstream default playwrightCliVersion
func resolvePatchrightVersion() string {
...
func NewDriver(options ...*RunOptions) (*PlaywrightDriver, error) {
	...
	version := resolvePatchrightVersion()
