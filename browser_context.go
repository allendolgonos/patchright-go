...
func (b *browserContextImpl) AddInitScript(script Script) error {
	// Ensure injection route is installed before adding init script
	_ = b.installInjectRoute()
	var source string
...
func (b *browserContextImpl) ExposeBinding(name string, binding BindingCallFunction, handle ...bool) error {
	// Ensure injection route is installed before exposing binding
	_ = b.installInjectRoute()
	needsHandle := false
	if len(handle) == 1 {
...
// installInjectRoute installs a high-priority catch-all route that allows the driver to inject
// initialization script via an internal URL for document requests. Subsequent calls are no-ops.
func (b *browserContextImpl) installInjectRoute() error {
	if b.routeInjecting {
		return nil
	}
	handler := func(route Route) {
		defer func() { _ = recover() }()
		req := route.Request()
		url := req.URL()
		if req.ResourceType() == "document" && (strings.HasPrefix(url, "http:") || strings.HasPrefix(url, "https:")) {
			proto := "http"
			if strings.HasPrefix(url, "https:") {
				proto = "https"
			}
			u := fmt.Sprintf("%s://patchright-init-script-inject.internal/", proto)
			_ = route.Fallback(RouteFallbackOptions{URL: &u})
			return
		}
		_ = route.Fallback()
	}
	// Insert at highest priority
	if err := b.Route("**/*", handler); err != nil {
		return err
	}
	b.routeInjecting = true
	return nil
}
