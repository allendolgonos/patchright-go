...
// installInjectRoute installs a page-level route for init script injection if not already present.
func (p *pageImpl) installInjectRoute() error {
	if p.browserContext != nil && p.browserContext.routeInjecting {
		return nil
	}
	if p.routeInjecting {
		return nil
	...
	if err := p.Route("**/*", handler); err != nil {
		return err
	}
